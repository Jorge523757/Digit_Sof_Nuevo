...
**Tailwind CSS** is a utility-first CSS framework packed with classes like `flex`, `pt-4`, `text-center` and `rotate-90` that can be composed to build any design, directly in your markup. **DigitSoft Django** interface is designed using Tailwind CSS and its components are from the `flowbite` plugin. **Flowbite** is an open-source library of over 600+ UI components, sections, and pages built with the utility classes from Tailwind CSS and designed in Figma. **DigitSoft Django** uses webpack integration to generate static assets that are used in the application. `Webpack` is a free and open-source module bundler for JavaScript. It is made primarily for JavaScript, but it can transform front-end assets such as HTML, CSS, and images if the corresponding loaders are included. Webpack takes modules with dependencies and generates static assets representing those modules.
